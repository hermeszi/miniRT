...
/*   Updated: 2025/06/10 11:45:09 by jngew            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_ambient(char *line, t_scene *scene)
{
	...
	if (scene->ambient.ratio >= 0)
		print_error ("Multiple ambient light definitions");
	tokens = ft_split(line, ' ');
	if (!tokens || !tokens[1] || !tokens[2] || tokens[3])
		print_error ("Invalid ambient light format (A ratio R,G,B)");
	scene->ambient.ratio = ft_atof(tokens[1]);
	if (scene->ambient.ratio < 0.0 || scene->ambient.ratio > 1.0)
		print_error ("Ambient ratio must be between 0.0 and 1.0");
...
void	parse_camera(char *line, t_scene *scene)
{
	...
	if (scene->camera.fov >= 0)
		print_error ("Multiple camera definitions");
	tokens = ft_split(line, ' ');
	...
	if (scene->camera.fov < 0 || scene->camera.fov > 180)
		print_error("Camera FOV must be between 0 and 180");
	free_tokens(tokens);
}
...
void	parse_light(char *line, t_scene *scene)
{
	...
	if (scene->light.brightness >= 0)
		print_error ("Multiple light definitions");
	tokens = ft_split(line, ' ');
	...
	if (scene->light.brightness < 0.0 || scene->light.brightness > 1.0)
		print_error ("Light brightness must be between 0.0 and 1.0");
	if (token_count > 3)
...
void	parse_color(char *str, t_color *color)
{
	...
	if (!rgb || !rgb[0] || !rgb[1] || !rgb[2] || rgb[3])
	{
		if (rgb)
			free_tokens(rgb);
		print_error ("Invalid color format (R,G,B)");
	}
